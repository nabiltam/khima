    ...
    if (hasError) {
      ...
      return (
        <div className="min-h-screen bg-[#F5F5F5] flex items-center justify-center p-6 text-right" dir="rtl">
          <div className="bg-white p-12 rounded-[3rem] border border-[#1A1A1A]/5 max-w-md w-full shadow-2xl space-y-6">
            ...
            <div className="space-y-2 text-center">
              <h2 className="text-3xl font-bold text-[#1A1A1A]">عذراً! حدث خطأ</h2>
              <p className="text-[#1A1A1A]/60 font-medium">{errorMessage}</p>
            </div>
            <button
              onClick={() => window.location.reload()}
              className="w-full py-4 bg-[#1A1A1A] text-white rounded-2xl font-bold hover:bg-[#1A1A1A]/90 transition-all shadow-xl shadow-[#1A1A1A]/20"
            >
              إعادة تحميل الصفحة
            </button>
          </div>
